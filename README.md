...
Иногда встает необходимость синхронизации файлов между двумя серверами для поддержания "репликации" файлов на обоих серверах. Данную задачу нам может помочь решить RSYNC, который можно настроить в роли службы с конфигурационными файлами (это мы рассматривать не будем), а так же с помощью планировщика задач Cron и создания скрипта запускающего синхронизацию.
...
Опции rsync -avz --update --chown=www-data позволяют отображать процесс передачи файлов, обновлять при изменении и задавать владельца файлов. В данном решении осуществляется копирование файлов и каталогов в каталоге веб-сервера и владельца www-datа.
